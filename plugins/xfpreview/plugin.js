/**
 * xfpreview 插件
 * 功能：增强「预览」功能。本插件拦截 preview 命令，改用 XfEditor.openPreview 渲染当前内容。
 * 说明：
 *   1）样式完全复用 XfEditor.getStandaloneCss()（examples/js/xf.js 中统一维护的
 *      「基础 + 特效」样式），确保预览与「导出 / getHtml」使用同一套样式，展示一致；
 *   2）XfEditor.openPreview 内部「优先尝试新窗口，失败（弹窗拦截 / 跨域沙箱抛
...
( function() {
    ...
    XfEditor.plugins.add( 'xfpreview', {
        init: function( editor ) {
            ...
            function doPreview( editor ) {
                // 统一交由 XfEditor.openPreview 处理：它内部会「优先尝试新窗口，
                // 失败（弹窗拦截 / 跨域沙箱抛 SecurityError）则自动回退到同源模态浮层」，
                // 彻底解决「Blocked a frame ... cross-origin」与跳出当前网页的问题。
                if ( typeof window.XfEditor === 'object' && typeof XfEditor.openPreview === 'function' ) {
                    XfEditor.openPreview( editor, '内容预览 - xfTextEditor' );
                    return;
                }
                // 兜底：未加载 js/xf.js 时，仍尽量渲染「完整自包含」预览（含基础 + 特效样式，
                // 图标 / 图片等用 XfEditor.getUrl 解析到编辑器基址，避免导出后丢失），
                // 并以 _blank 新开窗口（避免复用同名窗口造成跳转到其它页面）。
                var effectCss = ( XfEditor.tools && XfEditor.tools.xfEffectsCss ) ? XfEditor.tools.xfEffectsCss : '';
                var baseCss =
